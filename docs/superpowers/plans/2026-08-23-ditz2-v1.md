...
- **Everything environment-specific lives under `facebook/`.** the export tooling strips that directory by default, so it never reaches a public export — no manual scrubbing on extraction. Nothing outside `facebook/` may reference an internal host, an a large monorepo path, or an internal registry.
...
## Environment Bootstrap (an internal development host only)

The system `node` is 16.20.2 (EOL) and `/usr/local/bin/npm` is a wrapper that blocks all installs. `registry.npmjs.org` is unreachable. Three facts make this work:

- `https://registry.npmjs.org` is a transparent mirror of the public npm registry — reachable directly, no proxy, no auth.
- The real npm is `/usr/bin/npm`.
- a large monorepo vendors Node 21.4.0 at `third-party/node/v21.4.0/node`.

`the local bootstrap script` (created in Task 1) wires these up via environment variables only, so nothing environment-specific enters `package.json`. Outside that environment, ignore this file and use any Node >= 20.
...
ditz2/
  ...
  vitest.config.ts
  .gitignore              node_modules/ dist/ .npmrc .tooling/
  .nvmrc                  20
  README.md
  facebook/
    the local bootstrap script           internal-only: PATH + npm_config_registry.
                          the export tooling strips facebook/ by default, so this never
...
**Files:**
- Create: `package.json`, `tsconfig.json`, `vitest.config.ts`, `.gitignore`, `.nvmrc`, `the local bootstrap script`
...
- [ ] **Step 1: Create `the local bootstrap script`**

```bash
#!/bin/bash
# an internal development host only. Source this, do not execute it:  source the local bootstrap script
# Outside that environment this file is unnecessary — use any Node >= 20 and the public registry.
_ditz2="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
_root="$(cd "$_ditz2/../../../.." && pwd)"
_node="$_root/third-party/node/v21.4.0"

if [ ! -x "$_node/node" ]; then
  echo "bootstrap: no vendored node at $_node — skipping" >&2
else
  # A shim dir so /usr/bin/npm (shebang: env node) resolves to Node 21, not the
  # EOL system Node 16. /usr/local/bin/npm is a wrapper that refuses to install.
  mkdir -p "$_ditz2/.tooling/bin"
  ln -sf "$_node/node" "$_ditz2/.tooling/bin/node"
  ln -sf /usr/bin/npm  "$_ditz2/.tooling/bin/npm"
  ln -sf /usr/bin/npx  "$_ditz2/.tooling/bin/npx"
  export PATH="$_ditz2/.tooling/bin:$PATH"
fi

# Mirror of the public npm registry. An env var rather than a committed .npmrc
# so npm never records the mirror in package-lock.json (see Task 15 Step 6).
...
- [ ] **Step 3: Create `tsconfig.json`, `vitest.config.ts`, `.gitignore`, `.nvmrc`**
...
```bash
sl add package.json tsconfig.json vitest.config.ts .gitignore .nvmrc \
  the local bootstrap script src/core/clock.ts src/core/id.ts src/core/id.test.ts \
  --reason "scaffold ditz2 node project - sl help add"
sl commit --reason "commit ditz2 scaffold and id generation - sl help commit" \
  -m "ditz2: project scaffold and UUIDv7 id generation"
```

Do NOT `sl add` `package-lock.json` yet — Task 15 decides whether it is committed.
...
```bash
sl add src/core/errors.ts src/core/types.ts src/core/errors.test.ts \
  --reason "add ditz2 error and domain types - sl help add"
sl commit --reason "commit ditz2 error and domain types - sl help commit" \
...
```bash
sl commit --reason "commit ditz2 prefix resolution - sl help commit" \
...
```bash
sl add src/core/log.ts src/core/log.test.ts \
  --reason "add ditz2 log grammar - sl help add"
sl commit --reason "commit ditz2 log grammar - sl help commit" \
...
```bash
sl add src/core/serialize.ts src/core/serialize.test.ts \
  --reason "add ditz2 issue serialization - sl help add"
sl commit --reason "commit ditz2 serialization - sl help commit" \
...
```bash
sl add src/core/validate.ts src/core/validate.test.ts \
  --reason "add ditz2 validation - sl help add"
sl commit --reason "commit ditz2 validation - sl help commit" \
...
```bash
sl add src/core/mutate.ts src/core/mutate.test.ts \
  --reason "add ditz2 pure transitions - sl help add"
sl commit --reason "commit ditz2 transitions - sl help commit" \
 ...
 * and the local VCS, and inert outside a repository — so no VCS detection is needed.
 */
 ...
 * Returns "Name <email>" or null if neither git nor the local VCS can tell us.
 */
export function probeVcsIdentity(cwd: string): string | null {
  ...
  // the local VCS stores the whole "Name <email>" string in one key.
...
```bash
sl add src/store/root.ts src/store/config.ts src/store/identity.ts src/store/project.test.ts \
  --reason "add ditz2 store project layer - sl help add"
sl commit --reason "commit ditz2 store project layer - sl help commit" \
...
```bash
sl add src/store/issues.ts src/store/issues.test.ts \
  --reason "add ditz2 issue storage - sl help add"
sl commit --reason "commit ditz2 issue storage - sl help commit" \
...
```bash
sl add src/render/json.ts src/render/human.ts src/render/render.test.ts \
  --reason "add ditz2 render layer - sl help add"
sl commit --reason "commit ditz2 render layer - sl help commit" \
...
```bash
sl add src/cli/context.ts src/cli/main.ts src/cli/init.ts tests/helpers.ts tests/cli/init.test.ts \
  --reason "add ditz2 cli foundation and init - sl help add"
sl commit --reason "commit ditz2 cli foundation - sl help commit" \
...
```bash
sl add src/cli/message.ts src/cli/add.ts src/cli/show.ts tests/cli/add-show.test.ts \
  --reason "add ditz2 add and show commands - sl help add"
sl commit --reason "commit ditz2 add and show - sl help commit" \
...
```bash
sl add src/cli/filters.ts src/cli/list.ts src/cli/grep.ts tests/cli/list-grep.test.ts \
  --reason "add ditz2 list and grep - sl help add"
sl commit --reason "commit ditz2 list and grep - sl help commit" \
...
```bash
sl add src/cli/set.ts src/cli/close.ts src/cli/comment.ts tests/cli/mutate.test.ts \
  --reason "add ditz2 mutation commands - sl help add"
sl commit --reason "commit ditz2 mutation commands - sl help commit" \
...
`README.md` must stay publishable: no internal hostnames, no a large monorepo paths, no mention of `the local bootstrap script`. Put the internal bootstrap in a **separate `facebook/README.md`** instead — `source the local bootstrap script` and why (system Node is 16, the public registry is unreachable, `registry.npmjs.org` mirrors it). the export tooling strips it with the rest of `facebook/`.
...
```bash
sl add src/cli/edit.ts tests/cli/edit.test.ts README.md facebook/README.md package-lock.json \
  --reason "add ditz2 edit command and readme - sl help add"
sl commit --reason "commit ditz2 edit and readme - sl help commit" \
...
- Nothing outside `facebook/` mentions an internal host, an a large monorepo path, or an internal registry. Verify with:
  `grep -rIn --exclude-dir=node_modules --exclude-dir=dist --exclude-dir=facebook -E 'facebook\.net|internal-host|internal-url|a large monorepo|third-party' . | grep -v '^./docs/'`
  Expected: no output.
